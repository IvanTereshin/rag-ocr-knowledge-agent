...
Статус: базовый слой внедрён через `apps/api/src/upload-validation.ts`.
...
- `SESSION_COOKIE_DOMAIN` оставлять пустым для host-only cookie; задавать только если осознанно нужен общий cookie domain.
...
Статус: опциональный ClamAV/clamd scan внедрён через `apps/api/src/virus-scan.ts`; включается `UPLOAD_SCAN_ENABLED=true`.
